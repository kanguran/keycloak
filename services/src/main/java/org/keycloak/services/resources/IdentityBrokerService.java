...
public class IdentityBrokerService implements IdentityProvider.AuthenticationCallback {
    ...
    private void checkRealm() {
        if (!realmModel.isEnabled()) {
            ...
            throw new ErrorPageException(session, Status.BAD_REQUEST, Messages.REALM_NOT_ENABLED);
        }
    }
    private ClientModel checkClient(String clientId) {
        if (clientId == null) {
            event.error(Errors.INVALID_REQUEST);
            throw new ErrorPageException(session, Status.BAD_REQUEST, Messages.MISSING_PARAMETER, OIDCLoginProtocol.CLIENT_ID_PARAM);
        }
        event.client(clientId);
        ClientModel client = realmModel.getClientByClientId(clientId);
        if (client == null) {
            event.error(Errors.CLIENT_NOT_FOUND);
            throw new ErrorPageException(session, Status.BAD_REQUEST, Messages.INVALID_REQUEST);
        }
        if (!client.isEnabled()) {
    ...
    public Response clientInitiatedAccountLinking(@PathParam("provider_id") String providerId,
        ...
        for (AuthenticatedClientSessionModel cs : cookieResult.getSession().getAuthenticatedClientSessions().values()) {
            if (cs.getClient().getClientId().equals(clientId)) {
                ...
                try {
                    ...
                    throw new ErrorPageException(session, Status.INTERNAL_SERVER_ERROR, Messages.UNEXPECTED_ERROR_HANDLING_REQUEST);
                }
        ...
        try {
            ...
            return redirectToErrorPage(authSession, Status.INTERNAL_SERVER_ERROR, Messages.COULD_NOT_SEND_AUTHENTICATION_REQUEST, e, providerId);
        } catch (Exception e) {
            return redirectToErrorPage(authSession, Status.INTERNAL_SERVER_ERROR, Messages.UNEXPECTED_ERROR_HANDLING_REQUEST, e, providerId);
        }
        return redirectToErrorPage(authSession, Status.INTERNAL_SERVER_ERROR, Messages.COULD_NOT_PROCEED_WITH_AUTHENTICATION_REQUEST);
    }
    @POST
    ...
    public Response performLogin(@PathParam("provider_id") String providerId,
        ...
        try {
            ...
            return redirectToErrorPage(Status.BAD_GATEWAY, Messages.COULD_NOT_SEND_AUTHENTICATION_REQUEST, e, providerId);
        } catch (Exception e) {
            return redirectToErrorPage(Status.INTERNAL_SERVER_ERROR, Messages.UNEXPECTED_ERROR_HANDLING_REQUEST, e, providerId);
        }
        return redirectToErrorPage(Status.INTERNAL_SERVER_ERROR, Messages.COULD_NOT_PROCEED_WITH_AUTHENTICATION_REQUEST);
    }
    ...
    private Response getToken(String providerId, boolean forceRetrieval) {
        ...
        try {
            ...
            return redirectToErrorPage(Status.BAD_GATEWAY, Messages.COULD_NOT_OBTAIN_TOKEN, e, providerId);
        }  catch (Exception e) {
            return redirectToErrorPage(Status.BAD_GATEWAY, Messages.UNEXPECTED_ERROR_RETRIEVING_TOKEN, e, providerId);
        }
    }
    public Response authenticated(BrokeredIdentityContext context) {
        ...
        if (federatedUser == null) {
            ...
            logger.info("XXXX Redirecting to flow for firstBrokerLogin");
    ...
    public Response validateUser(AuthenticationSessionModel authSession, UserModel user, RealmModel realm) {
        if (!user.isEnabled()) {
            ...
            return ErrorPage.error(session, authSession, Status.BAD_REQUEST, Messages.ACCOUNT_DISABLED);
        }
    ...
    private Response afterFirstBrokerLogin(AuthenticationSessionModel authSession) {
        try {
            ...
            return redirectToErrorPage(authSession, Status.INTERNAL_SERVER_ERROR, Messages.IDENTITY_PROVIDER_UNEXPECTED_ERROR, e);
        }
    }
    ...
    public Response afterPostBrokerLoginFlow(@QueryParam(LoginActionsService.SESSION_CODE) String code,
        ...
        try {
            ...
            return redirectToErrorPage(authenticationSession, Status.INTERNAL_SERVER_ERROR, Messages.IDENTITY_PROVIDER_UNEXPECTED_ERROR, e);
        }
    }
    ...
    private boolean shouldPerformAccountLinking(AuthenticationSessionModel authSession, UserSessionModel userSession, String providerId) {
        ...
        if (linkingValid) {
            ...
            throw new ErrorPageException(session, Status.BAD_REQUEST, Messages.BROKER_LINKING_SESSION_EXPIRED);
        }
    }
    private Response performAccountLinking(AuthenticationSessionModel authSession, UserSessionModel userSession, BrokeredIdentityContext context, FederatedIdentityModel newModel, UserModel federatedUser) {
        ...
        if (!authenticatedUser.hasRole(this.realmModel.getClientByClientId(Constants.ACCOUNT_MANAGEMENT_CLIENT_ID).getRole(AccountRoles.MANAGE_ACCOUNT))) {
            return redirectToErrorPage(authSession, Status.FORBIDDEN, Messages.INSUFFICIENT_PERMISSION);
        }
    ...
    private Response redirectToErrorWhenLinkingFailed(AuthenticationSessionModel authSession, String message, Object... parameters) {
        if (authSession.getClient() != null && authSession.getClient().getClientId().equals(Constants.ACCOUNT_MANAGEMENT_CLIENT_ID)) {
            ...
            return redirectToErrorPage(authSession, Status.BAD_REQUEST, message, parameters); // Should rather redirect to app instead and display error here?
        }
    }
    ...
    private AuthenticationSessionModel parseSessionCode(String code, String clientId, String tabId) {
        if (code == null || clientId == null || tabId == null) {
            ...
            Response staleCodeError = redirectToErrorPage(Status.BAD_REQUEST, Messages.INVALID_REQUEST);
    ...
    private Response redirectToErrorPage(AuthenticationSessionModel authSession, Status status, String message, Object ... parameters) {
        return redirectToErrorPage(authSession, status, message, null, parameters);
    }
    private Response redirectToErrorPage(Status status, String message, Object ... parameters) {
        return redirectToErrorPage(null, status, message, null, parameters);
    }
    private Response redirectToErrorPage(Status status, String message, Throwable throwable, Object ... parameters) {
        return redirectToErrorPage(null, status, message, throwable, parameters);
    }
    private Response redirectToErrorPage(AuthenticationSessionModel authSession, Status status, String message, Throwable throwable, Object ... parameters) {
        if (message == null) {
